...
```
故事素材 → 剧本创作 → 图像提示词 → 执行稿拆解 → 角色场景图 → 视频生成 → ffmpeg后期封装
```
...
| 1 | 故事素材 | 用户输入 |
| 2 | 剧本创作 | 工作流1：剧本创作 |
| 3 | 图像提示词 | 工作流2：图像提示词生成器 |
| 4 | 执行稿拆解 | 工作流3：短视频故事执行稿 |
| 5 | 角色场景图 | Nano Banana 2（角色三视图、场景图等）|
| 6 | 视频生成 | Grok Imagine / 剪映 |
| 7 | ffmpeg后期封装 | ffmpeg 拼接成片 |
